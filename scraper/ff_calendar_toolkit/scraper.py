    ...
    def parse_table(self, driver: webdriver.Chrome, month_name: str) -> list[dict]:
        ...
        for row in table.find_elements(By.TAG_NAME, "tr"):
            ...
            for element in row.find_elements(By.TAG_NAME, "td"):
                ...
                # Use substring matching to find the appropriate cell key
                class_name_key = None
                # Sort keys by length descending to match more specific classes first
                for candidate_class in sorted(ALLOWED_ELEMENT_TYPES.keys(), key=len, reverse=True):
                    if candidate_class in class_name:
                        class_name_key = ALLOWED_ELEMENT_TYPES[candidate_class]
                        break
                        
                if not class_name_key:
                    continue

                if class_name_key == "impact":
                    ...
                    if text_content:
                        row_data[class_name_key] = text_content
                    else:
